...
<template>
  <CustomNuxtLink v-if="!link.includes('-')" :to="link">
    <NCard class="content-card">
      <template #cover>
        <NuxtPicture v-if="image" class="content-card-cover-img" :src="image" />
      </template>
      <template #header>
        <NFlex align="center">
          <NTag
          ...
          <NP class="content-card-date">{{ format(parseISO(date), 'dd/MM/yyyy') }}</NP>
        </NFlex>
      ...
      <template #default>
        <NEllipsis :line-clamp="1" :tooltip="false">
          <NP class="content-card-description">{{ description }}</NP>
        </NEllipsis>
      </template>
    </NCard>
  </CustomNuxtLink>
  <NSpin v-else class="loader" />
</template>
